...
Active runtime profile for this workspace. Written by `init` skill on
first run (or on runtime change) and ratified by the Owner. Read by
`learn` and `audit` to look up active memory caps per R1.

T2 (Owner-ratified). Default if this file is absent: `lean`.
...
Full cap table at `3-control/foundation/use-driven-memory.md §R1`.
Summary:
...
- Owner judgment ("we're using more journal detail; bump to standard")
